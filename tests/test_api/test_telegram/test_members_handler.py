...
def handler(faker) -> ChatMembersHandler:
    chats = {faker.random.randrange(1, 10 ** 6) * (-1),
             faker.random.randrange(1, 10 ** 6) * (-1)}
    return ChatMembersHandler(chats=chats)
...
def _new_chat_member(faker, username=None, user_id=None, is_bot=False):
    ...
    member.username = username or faker.pyint()
    return member
...
def test__do_nothing__when__bot_join_moitored_chat(
    ...
    update = join_update(member, chat_id=tuple(handler.chats)[0])
...
def test__do_nothing__when__exists_user__join__monitored_chat(
...
def test__do_nothing__when__unknown_user__left__monitored_chat(
    ...
    member = new_chat_member()
    update = left_update(member, chat_id=tuple(handler.chats)[0])
...
def test__goodbye__when__exists_user__left__monitored_chat(
    db_session, faker, adapter, handler, people, new_chat_member, bot,
    left_update,
):
    member = new_chat_member(user_id=people.telegram_login)
